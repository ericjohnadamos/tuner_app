...
@implementation TMOPitchDetector
{
  long kBufferSize;
}

#pragma mark - Shared detector

/* Setup the singleton TMOPitchDetector */
+ (id) sharedDetector
{
  static TMOPitchDetector *sharedDetector = nil;
...
- (id) init
{
  if (self = [super init]) NSLog(@"Should only call once!");
...
#pragma mark - Initialize Pitch Detector
...
- (size_t) ASBDForSoundMode
{
  ...
  asbd.mBytesPerFrame = (UInt32)(bytesPerSample * asbd.mChannelsPerFrame);
...
{
  /* Base MUST be 2! */
  maxFrames = (UInt32)(kBufferSize * 2);
...
- (void) createMicrophone
{
  ...
  /* Open the Graph for configuration, note: graph has to be opened before
  ...
  /* Enable input and disable output, setup the callback function to analysis
   * audio data
   */
...
#pragma mark - Turn On Or OFF Microphone
...
- (void) TurnOffMicrophone
{
  ...
  if (processingGraph) AUGraphStop(processingGraph);
}
...
{
  TMOPitchDetector* THIS = (__bridge TMOPitchDetector*) inRefCon;
  
  /*------------Obtain Engine Parameters-----------------*/
  
  TMOMainViewController* mainViewPitch = THIS->mainViewPitch;
  ...
  else
  {
    ...
    if (mainViewPitch != nil)
    {
      [mainViewPitch updateToFrequency: frequency];
    }
    ...
    if (percentageOfOverlap>0 && percentageOfOverlap<100)
    {
      ...
      sizeOfOverlap = (UInt32)(bufferCapacity - start);
    ...
    remain = (int)(inNumberFrames - read);
    if (remain > 0)
    {
      UInt32 afterAddRemain = (UInt32)(THIS->index + remain);
      ...
      else // really need to discard some old samples, so that new samples can be process
      {
        ...
        start = (UInt32)(THIS->index - (bufferCapacity - remain));
        sizeOfOverlap = (UInt32)(THIS->index - start);
...
/* ConvertInt16ToFloat(THIS, dataBuffer, outputBuffer, bufferCapacity); */
void ConvertInt16ToFloat(TMOPitchDetector* THIS,
                         void*             buf,
                         float*            outputBuf,
                         size_t            capacity)
{
  ...
  outFormat.mBitsPerChannel = (UInt32)(8 * bytesPerSample);
  outFormat.mFramesPerPacket = 1;
  outFormat.mChannelsPerFrame = 1;
  outFormat.mBytesPerPacket
    = (UInt32)(bytesPerSample * outFormat.mFramesPerPacket);
  outFormat.mBytesPerFrame
    = (UInt32)(bytesPerSample * outFormat.mChannelsPerFrame);
...
{
  if (midiNote<=-1) return @"NIL";
  
  NSArray* noteStrings = [[NSArray alloc] initWithObjects:
                          @"C",
                          @"C#",
                          @"D",
                          @"D#",
...
/* FIXME: Seem should not use this function, as it is very time comsumption need
 * +100ms
 */
void writeToDocFile(float* data, int size, NSString* fileName)
{
  ...
  if (![[NSFileManager defaultManager] fileExistsAtPath: dataFile])
  {
    ...
    [[NSFileManager defaultManager] createFileAtPath: dataFile
                                            contents: nil
                                          attributes: nil];
  }
  
  NSFileHandle* fileHandle
    = [NSFileHandle fileHandleForWritingAtPath: dataFile];
  
  for (int i=0; i<size && data+i != nil; i++)
  {
    [fileHandle writeData:
     [[NSString stringWithFormat: @"%f ", data[i]]
